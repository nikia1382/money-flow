...
describe('AccountsService', () => {
  ...
  it('should add a new account', () => {
    ...
    expect(service.accounts().length).toBe(4);

    const addedAccount = service.accounts().find((account) => account.name === 'Test Account');

    expect(addedAccount).toBeTruthy();

    expect(addedAccount?.balance).toBe(10_000_000);
  });
  ...
  it('should update an account', () => {
    ...
    const result = service.accounts().find((item) => item.id === account.id);

    expect(result?.name).toBe('Updated Account');

    expect(result?.balance).toBe(99_000_000);
  });
  ...
  it('should save accounts to localStorage', () => {
    ...
    expect(parsed.some((account: Account) => account.name === 'Saved Account')).toBeTrue();
  });
});
