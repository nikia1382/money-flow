...
export class DataTable<T extends object> {
  ...
  @Input()
  emptyTitleKey = 'common.table.emptyTitle';

  @Input()
  emptySubtitleKey = 'common.table.emptySubtitle';

  @Output()
  editRow = new EventEmitter<T>();

  @Output()
  deleteRow = new EventEmitter<T>();
  ...
  getTranslatedValueKey(column: DataTableColumn<T>, value: unknown): string {
    if (!column.valueKeyPrefix || typeof value !== 'string') {
      return '';
    }
